...
# Modified file from github.com/DiscordHooks/travis-ci-discord-webhook
...
WEBHOOK_DATA='{
  ...
  "embeds": [ {
    "color": '$EMBED_COLOR',
    "author": {
      "name": "'"$REPO_NAME"' - '"$TRAVIS_JOB_NAME"' '"$STATUS_MESSAGE"' - Job #'"$TRAVIS_JOB_NUMBER"' (Build #'"$TRAVIS_BUILD_NUMBER"')",
